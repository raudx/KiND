...
def isSentenceConsumed(words):
    if (len(words) == 1):
        ...
        if (words[index] == "null" and words[index + 1] == "and" and words[index + 2] == "null" and index + 3 == len(words)):
            return True
        else:
            return False
...
def extractKeywordsFromScratched(scratch):
    ...
    for i in range(len(sentences)):
        ...
        array.append([stringJoiner(sentences[i].split()), twoDkeyArray[i]])

    return array  # print extractKeywordsFromScratched("Print Add two to 5.0 and 4.0. Multiply of 3 and 5..")
...
def retAllKeysNotKeys():
    ...
    # appending keys -----
    for i in range(len(io_keys)):
        all_keys.append(io_keys[i])

    for i in range(len(math_keys)):
...
def isProbableVar(word):
    ...
    if ( word not in all_keys and word not in not_keys ):
        return True and not (isNumber(word) or (word[0] == "\"" and word[-1] == "\"") or word == "," or word == "and")
...
def stringJoiner(words) :
    ...
    for i in range(len(words)) :
        ...
        if (in_str == False) :
            ...
            if (words[i][-1] == "\"") :
                in_str = False
                out_str[-1] = out_str[-1] + " " + words[i]

            else :
                out_str[-1] = out_str[-1] + " " + words[i]

    # print 393, words, out_str
    return out_str


def checkPartialMatch(rule_list) :
    for rule in rule_list :
        if (rule.partial_match == True) :
            print "---SENTENCE PARTIALLY MATCHED---"
            break